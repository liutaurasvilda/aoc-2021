...
final class Day04 {
    ...
    public static void main(String[] args) {
        ...
        List<String> input = ResourceReader.asString("day04.txt");
        ...
        for (int num : numbers) {
            for (BingoBoard bingoBoard : bingoBoards) {
                bingoBoard.mark(num);
                if (bingoBoard.isWinner() && bingoBoard.wereNotWinnerBefore()) {
                    winnerResults.add(bingoBoard.getScore());
                    bingoBoard.setWereWinnerBefore();
                }
            }
        }
